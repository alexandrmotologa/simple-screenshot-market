...
// ── Inline number input ───────────────────────────────────────────────────────
function NumInput({
  value, onChange, min, max, unit, width = "w-12",
}: {
  value: number;
      ...
      {editing ? (
        <div className="relative flex items-center">
          <input
            ...
            className={cn(
              "bg-secondary border border-primary/50 rounded px-1 py-0.5 text-xs font-mono text-center outline-none [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none",
              width
            )}
          />
        ...
        <button
          ...
          className={cn("text-xs font-mono text-center hover:bg-secondary rounded px-1.5 py-0.5 tabular-nums text-foreground font-medium transition-colors", width)}
          title="Click to edit"
        >
